...
import MyAlgoConnectWallet from "algorand-session-wallet/dist/wallets/myalgoconnect";
import {PermissionCallback, SignedTxn, Wallet} from "algorand-session-wallet";
...
export class WalletsConnectService {
  ...
  async doSign(defaultAcct: string, txns: Transaction[]): Promise<SignedTxn[]> {
    ...
    for (let x = 0; x < signedTxns.length; x++) {
      if (typeof signedTxns[x] === "number") { // @ts-ignore
